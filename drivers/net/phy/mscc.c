				...
				u8 page, u8 reg_addr, u16 *data)
{
	...
	*data = 0xDEAD;
	if (page != 0) {
        reg_val = phy_read_paged(phydev, page, reg_addr);
        if (reg_val < 0)
                return reg_val;
	} else {
...
static int vsc85xx_phy_write_reg(struct phy_device *phydev,
				 u8 page, u8 addr, u16 data)
{
	int rc;

	if (page != 0) {
		rc = phy_select_page(phydev, page);
		if (rc < 0)
			return rc;
		rc = phy_write(phydev, addr, data);
		rc = phy_restore_page(phydev, rc, rc);
	} else {
		rc = phy_write(phydev, addr, data);
	}
			       ...
			       struct ethtool_tunable *tuna, void *data)
{
	...
	case ETHTOOL_PHY_READ_REG:
	{
		...
		memcpy(&tmp, data, sizeof(u16));
		page = (u8)((tmp >> 8) & 0xff);
		addr = (u8)(tmp & 0xff);
		return vsc85xx_phy_read_reg(phydev, page, addr, (u16 *)data);
	}
#endif
			       ...
			       const void *data)
{
	...
	case ETHTOOL_PHY_WRITE_REG:
	{
		...
		memcpy(&tmp, data, sizeof(u32));
		addr = (u8)(tmp & 0xff);
		page = (u8)((tmp >> 8) & 0xff);
		value = (u16)((tmp >> 16) & 0xffff);
		return vsc85xx_phy_write_reg(phydev, page, addr, value);
	}
#endif
	case ETHTOOL_PHY_DOWNSHIFT:
